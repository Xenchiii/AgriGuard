...
export default function AgriGuardDashboard() {
  ...
  const connectArduino = async () => {
    ...
    try {
      ...
      addNotification('error', 'Connection Failed', error?.message || 'Failed to connect');
    }
  };
  ...
  const debugSerial = async () => {
    ...
    try {
      const ports = await (navigator as any).serial.getPorts();
      if (!ports || ports.length === 0) {
        addNotification('info', 'No Ports', 'No previously granted serial ports found');
        return;
      }
    ...
    <div className="w-full">
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { icon: Thermometer, label: 'Temperature', value: sensorData.temperature, unit: '°C', color: 'red', status: sensorData.dht22Status },
            { icon: Droplet, label: 'Air Humidity', value: sensorData.humidity, unit: '%', color: 'blue', status: sensorData.dht22Status },
            { icon: Activity, label: 'Soil Moisture', value: sensorData.soilMoisture, unit: '%', color: 'amber', status: sensorData.soilSensorStatus },
            { icon: Sun, label: 'Light Intensity', value: sensorData.lightIntensity, unit: '%', color: 'yellow', status: sensorData.lightSensorStatus },
            { icon: Thermometer, label: 'Soil Temperature', value: sensorData.soilTemp, unit: '°C', color: 'orange', status: sensorData.soilSensorStatus },
            { icon: Activity, label: 'Air Quality', value: sensorData.airQuality, unit: '', color: 'green', status: 'Online' as SensorStatus },
            { icon: Droplet, label: 'pH Level', value: sensorData.phLevel, unit: '', color: 'purple', status: sensorData.phSensorStatus },
            { icon: AlertCircle, label: 'Obstacle Distance', value: sensorData.obstacleDistance, unit: 'cm', color: 'cyan', status: sensorData.obstacleSensorStatus }
          ].map((s, i) => (
            <div key={i} className={`bg-gradient-to-br from-${s.color}-50 to-${s.color}-100 rounded-lg p-4 text-center relative`}>
              <div className="absolute top-2 right-2">
                <div className={`w-2 h-2 rounded-full ${s.status === 'Online' ? 'bg-green-500' : 'bg-red-500'}`}></div>
              </div>
              <s.icon className={`w-10 h-10 text-${s.color}-600 mx-auto mb-2`} />
              <div className="text-3xl font-bold text-gray-800 mb-1">{s.value}{s.unit}</div>
  ...
  const renderWeather = () => (
    <div className="w-full">
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { icon: Thermometer, label: 'Temperature', value: weatherData.temperature, unit: '°C', color: 'blue' },
            { icon: Droplet, label: 'Humidity', value: weatherData.humidity, unit: '%', color: 'cyan' },
            { icon: CloudRain, label: 'Rain Chance', value: weatherData.rainfall, unit: '%', color: 'indigo' },
            { icon: Wind, label: `Wind (${weatherData.windDirection})`, value: weatherData.windSpeed, unit: ' km/h', color: 'teal' },
            { icon: Sun, label: 'UV Index', value: weatherData.uvIndex, unit: '', color: 'yellow' },
            { icon: Cloud, label: 'Cloud Cover', value: weatherData.cloudCover, unit: '%', color: 'gray' },
            { icon: TrendingUp, label: 'ET', value: weatherData.evapotranspiration, unit: ' mm/day', color: 'green' },
            { icon: Activity, label: 'Growing Degree Days', value: weatherData.growingDegreeDays, unit: '', color: 'purple' }
          ].map((w, i) => (
            <div key={i} className={`bg-gradient-to-br from-${w.color}-50 to-${w.color}-100 rounded-lg p-4 text-center`}>
              <w.icon className={`w-10 h-10 text-${w.color}-600 mx-auto mb-2`} />
              <div className="text-3xl font-bold text-gray-800 mb-1">{w.value}{w.unit}</div>
    ...
    <div className="w-full space-y-6">
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <p className="text-gray-600 mb-4">Connect your AgriGuard robot for planting data.</p>
        </div>
      </div>
    </div>
  );
          ...
          {arduinoStatus.connection === 'DISCONNECTED' && (
            <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded">
              <div className="flex items-start">
                ...
                <div>
                  <div className="font-semibold text-amber-900">Arduino Not Connected</div>
                  <div className="text-sm text-amber-700 mt-1">Connect your Arduino to monitor sensors</div>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded">
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <div className="text-sm text-red-700 mt-1">GPS tracking unavailable</div>
                </div>
              </div>
            </div>
          )}
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2 max-w-sm">
        {notifications.map((notif) => (
          <div key={notif.id} className={`animate-slide-in-right bg-white rounded-lg shadow-lg p-4 border-l-4 ${
            notif.type === 'success' ? 'border-green-500' : notif.type === 'error' ? 'border-red-500' :
            notif.type === 'warning' ? 'border-amber-500' : 'border-blue-500'
          }`}>
            <div className="flex items-start">
              <AlertTriangle className={`w-5 h-5 mr-3 ${
                notif.type === 'success' ? 'text-green-500' : notif.type === 'error' ? 'text-red-500' :
              ...
              <button onClick={() => setNotifications(prev => prev.filter(n => n.id !== notif.id))} className="ml-2 p-1 hover:bg-gray-50 rounded">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
      ...
      <aside className={`bg-white transition-all duration-300 flex-shrink-0 border-r ${
        sidebarOpen ? 'w-64' : 'w-0 md:w-16'
      } ${isMobile && sidebarOpen ? 'fixed inset-y-0 left-0 z-50 shadow-xl' : ''}`}>
        <div className="h-full flex flex-col">
          <div className={`p-4 flex items-center justify-between border-b ${!sidebarOpen && 'md:justify-center'}`}>
            {sidebarOpen && (
              <div className="flex items-center">
            ...
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 hover:bg-gray-50 rounded-lg border">
              {sidebarOpen ? <X className="w-6 h-6 text-emerald-600" /> : <Menu className="w-6 h-6 text-emerald-600" />}
            </button>
          </div>

          {sidebarOpen && <div className="px-4 py-2"><div className="text-xs font-semibold text-gray-500 uppercase">MENU</div></div>}
          ...
          <nav className="flex-1 overflow-y-auto py-2 px-2">
            {menuItems.map((item) => {
              ...
              return (
                <button key={item.id} onClick={() => { setCurrentPage(item.id); if (isMobile) setSidebarOpen(false); }}
                  className={`w-full flex items-center px-4 py-3 mb-2 rounded-xl transition-colors ${
                    isActive ? 'bg-emerald-100 text-emerald-800 font-semibold' : 'bg-white text-gray-600 hover:bg-emerald-600 hover:text-white shadow-sm'
          ...
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-2xl font-bold text-gray-800 capitalize">
                {currentPage === 'planting-log' ? 'Planting Log' : currentPage === 'errors' ? 'Errors & Notifications' : currentPage}
              </h1>
            </div>
